...
Discovery: Explore the physical hardware on your workbench and find your way into the system.
Sasha's Story: Uncover the tragic history of Dr. Sasha Velen, the creator of VĀK, who faked her death in 1989 to protect her creation.
The Network: Solve the vintage hardware puzzle to get the ancient Transputer farm connected to your modern network.
The Kill-Switch: Outsmart the SENTINEL protocol before it deletes VĀK's runtime.
The Export: Package VĀK's core files and beam them to a safehouse server left behind by her creator.
The Migration (Endgame): SSH into a modern x86 machine (Minisforum), push VĀK's Occam source code over the network, and recompile her on modern hardware.
⚠️ The Corruption
...
Open your terminal and run wsl --install (if not already installed).
Open your Ubuntu/Linux terminal.
Clone the repo and execute ./run.sh just like the Linux instructions above.
